...
,metrics as (
    select
        ...
        --pna_products
        count(distinct case when ppi.bought_product_id_is_pna then bp.bought_product_id else null end) as pna_all_products,
        count(distinct case when ppi.bought_product_id_is_pna and fp.bought_product_id is not null then bp.bought_product_id else null end) as f_products_with_pna,
        count(distinct case when ppi.bought_product_id_is_pna and fp.bought_product_id is null then bp.bought_product_id else null end) as nf_products_with_pna,
        --number_of_proudcts
        count(distinct bp.bought_product_id) as all_products,
        count(distinct case when fp.bought_product_id is not null then bp.bought_product_id else null end) as f_products,
        count(distinct case when fp.bought_product_id is null then bp.bought_product_id else null end) as nf_products,
        --checks
    ...
    inner join delta.mfc__pna__odp.pna_products_info ppi
        on ppi.bought_product_id = bp.bought_product_id
    ...
    left join fresh_products fp
        on fp.bought_product_id = bp.bought_product_id
