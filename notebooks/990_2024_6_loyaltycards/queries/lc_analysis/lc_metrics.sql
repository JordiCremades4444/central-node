...
,group_calendar_dates as (
    select
        calendar_date
    from unnest(sequence(date_add('day', {previous_days_start_date}, date({start_date})),date({end_date}),interval '1' day)) as dates (calendar_date)
    where 1=1
)
...
,customer_exposure as (
    select
        ...
        ,coalesce(lag(first_exposure_datetime) over (partition by allocation_key order by first_exposure_datetime desc), cast({end_date} as timestamp)) as end_time
...
,not_exposed_period as (
    select 
        customer_id, 
        'Not Exposed' as variant,  -- or any placeholder value you want to indicate no exposure
        cast(({start_date}) as timestamp) as start_time,
        fe.first_exposure_datetime AS end_time
    from first_exposure fe
)

,lc_groups as (
    select * from not_exposed_period
    union all
...
,n_orders_gmv_all_groceries as (
    select
        od.p_creation_date
        ,coalesce(lcg.variant, 'Not found') as variant
...
,n_orders_gmv_lc_stores as (
    ...
    left join lc_groups lcg
        ...
        and od.order_created_at < lcg.end_time
    where 1=1
        and store_id in (select store_id from loyalty_card_stores) --look at stores that showed at least a lc order
        and date(p_creation_date) in (select calendar_date from calendar_dates)
        and od.customer_id in (select customer_id from not_glovo_employees) --with the inner join is enough
        and od.order_final_status = 'DeliveredStatus'
    group by 1,2,3
...
,cvr as (
    ...
    where 1=1
        ...
        and sa.customer_id in (select customer_id from not_glovo_employees) --with the inner join is enough
    group by 1,2,3
)
...
select 
    ...
    cvr.p_creation_date
    ,cvr.store_id
    ,loyalty_card_stores.store_name
    ,cvr.variant
...
left join loyalty_card_stores 
    on cvr.store_id = loyalty_card_stores.store_id