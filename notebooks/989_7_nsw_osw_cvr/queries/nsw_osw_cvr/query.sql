with calendar_dates as (
    select
        calendar_date
    from unnest(sequence(date('2024-04-01'),date(current_date),interval '1' day)) as dates (calendar_date)
    where 1=1
)
...
, groups_calendar_dates as (
    select
        calendar_date
    from unnest(sequence(date('2023-01-01'),current_date,interval '1' day)) as dates (calendar_date)
    where 1=1
)
...
,order_events as (
    ...
    inner join delta.partner_stores_odp.stores_v2
        ...
        and stores_v2.store_subvertical3 in ('Groceries')
...
from funnel
where 1=1
    and p_creation_month is not null
