...
, non_top_partners as (
    select 
        ...
        count(distinct case when store_subvertical = 'QCPartners' then concat(cast(bp.order_id as varchar), '_', cast(bp.product_external_id as varchar)) else null end) as n_pairs_orders_external_id_in_fp_orders,
        count(distinct case 
    ...
    where true
        and not s.is_top_partner
    group by 1
)

select * from non_top_partners