...
select
    s.country_code,
    -- all groceries
    count(distinct case when store_subvertical = 'QCPartners' then bp.order_id else null end) as n_orders_all_groceries,
    count(distinct case when store_subvertical = 'QCPartners' and p.product_category_level_one in ('Produce','Ready to Consume','Meat / Seafood') then bp.order_id else null end) as n_orders_containing_fresh_products_all_groceries,
