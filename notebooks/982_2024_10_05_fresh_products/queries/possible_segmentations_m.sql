...
select
    ...
    count(distinct case when p.store_address_id is not null then s.store_id else null end) as n_store_id_m,
    1.00*count(distinct case when p.store_address_id is not null then s.store_id else null end)/count(distinct s.store_id) as perc_m
from stores s
left join delta.mfc_inventory_odp.products_v2 p
    on p.store_address_id = s.store_address_id
group by 1