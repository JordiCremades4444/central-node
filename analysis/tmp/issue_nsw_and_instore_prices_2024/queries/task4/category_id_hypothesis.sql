...
,all_funnel as (
    select
        ...
        count(distinct ce.dynamic_session_id) filter(where ce.category_id is null) as null_count_ce_dynamic_session_id,
        count(distinct swe.dynamic_session_id) filter(where swe.category_id is null) as null_count_swe_dynamic_session_id,
        count(distinct ce.dynamic_session_id) filter(where ce.category_id in ({list_category_id})) as groceries_count_ce_dynamic_session_id,
        count(distinct swe.dynamic_session_id) filter(where swe.category_id in ({list_category_id})) as groceries_count_swe_dynamic_session_id,        
    ...
    left join store_events as se --store impressions
        ...
        and se.creation_time > swe.creation_time
    ...
    left join order_events as oe --order events
        ...
        and oe.creation_time > se.creation_time
    ...
    left join instore_groups
        on coalesce(ce.customer_id, se.customer_id, oe.customer_id) = instore_groups.customer_id
        and ce.p_creation_date = instore_groups.p_event_date
    group by 1,2,3,4,5
)
