...
select 
    pna_orders_info.p_creation_date,
    pna_orders_info.order_city_code,
    experiment_group,
    count(*) as total_rows,
    count(distinct pna_orders_info.order_id) as total_orders,
    count_if(order_is_pna) as total_orders_with_pna ,
    sum(pnl_order_level.contribution_margin_eur) as pnl_cm_eur
...
left join delta.finance_financial_reports_odp.pnl_order_level AS pnl_order_level
    on pna_orders_info.order_id = pnl_order_level.order_id
where 1=1
    and pna_orders_info.p_creation_date >= date_add('day', -{more_less_days},  date('{start_test}'))
    and pna_orders_info.p_creation_date <= date_add('day', {more_less_days},  date('{end_test}'))
    and pna_orders_info.order_city_code in ({cities})
