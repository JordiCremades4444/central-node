...
,products as (
    select 
        ...
        sum(case
                ...
                when replaced_by_bought_product_id is not null and not qo.order_is_pna_cancellation and not pgv.bought_product_id_is_pna_replacement then bought_product_quantity-replacer_bought_product_quantity
            end
        ) as diff_units_removals,
        sum(bought_product_quantity) filter (where pgv.bought_product_id_is_pna_replacement) as originally_placed_units_delivered_orders_replacements,
...
select 
    order_country_code,
    month,
    coalesce(diff_units_removals,0) as diff_units_removals,
    coalesce(originally_placed_units_delivered_orders_replacements,0) as originally_placed_units_delivered_orders_replacements,
    ...
    1.0000*(coalesce(diff_units_removals,0)+coalesce(originally_placed_units_delivered_orders_replacements,0)+coalesce(originally_placed_units_pna_cancelled_orders,0))/(coalesce(originally_placed_units_delivered_orders,0)+coalesce(originally_placed_units_pna_cancelled_orders,0)) as perc_not_found
from products